...
class SmtpTest < Test::Unit::TestCase
  ...
  def setup
    ...
    FileUtils.cp_r("/etc/exim4/bytemark-vhost.d",File.join(@tempdir,"exim4"))
    FileUtils.cp_r("/etc/exim4/Makefile",File.join(@tempdir,"exim4"))
...
#    FileUtils.rm_rf(@tempdir) if @tempdir =~/^\/tmp/
  ...
  def do_acl_setup
    ...
    @acl_config['local_users'].each do |u|
      ...
      if u['crypt_password']
        File.open(File.join(mailbox, "password"),"w+"){|fh| fh.puts u['crypt_password']}
      else
