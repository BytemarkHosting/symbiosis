...
class DateTime
  ...
  #
  def iso8601
    format('%04i-%02i-%02i %02i:%02i', year, month, day, hour, min)
  end
...
module Symbiosis
  ...
  class Crontab
    ...
    def initialize(string_or_filename = '')
      ...
      if File.exist?(string_or_filename)
        ...
        @crontab = File.open(string_or_filename, &:read)
      else
    ...
    #
    def mail_output=(n)
      raise ArgumentError unless n.is_a?(TrueClass) || n.is_a?(FalseClass)
      @mail_output = n
    end
    ...
    #
    def test(t = Time.now)
      cron_env = @environment.merge(ENV) { |_k, o, _n| o }
      puts "Environment\n" + '-' * 72
      ...
      puts format('%-20s', 'Date') + 'Command'
      puts '-' * 72
      @records.each do |record|
        n = record.next_due(t)
        puts format('%-20s', (n.nil? ? '** NEVER **' : n.iso8601)) + record.command
      end
      puts '=' * 72
    end
    ...
    def run
      ...
      @records.select(&:ready?).each do |record|
        this_output = record.run
        next if this_output.empty?
    ...
    def parse(str)
      str.split($INPUT_RECORD_SEPARATOR).each do |line|
        line.chomp!
        ...
        if line =~ /\A([A-Z]+)\s*=\s*(.*)\Z/
          @environment[Regexp.last_match(1)] = Regexp.last_match(2)
        else
  ...
  class CrontabRecord
    ...
    #
    WDAY = %w(sun mon tue wed thu fri sat).freeze

    #
    # Month names that can be used in records.
    #
    MON  = %w(jan feb mar apr may jun jul aug sep oct nov dec).freeze
    ...
    #
    def self.parse(str)
      raise CrontabFormatError, "Badly formatted line: #{str.inspect}" unless str =~ /\A(?:@(?:#{SHORTCUTS.keys.join("|")})\s+|(?:\S+\s+){5})(.*)\Z$/
      ...
      wday = wday.downcase.gsub(/\b(#{WDAY.join("|")})[a-z]*/) do
        WDAY.index(Regexp.last_match(1))
      end

      # Same as above, but have to add one, as months start at 1 not 0.
      mon = mon.downcase.gsub(/\b(#{MON.join("|")})[a-z]*/) do
        MON.index(Regexp.last_match(1)) + 1
      end

      new(min, hour, mday, mon, wday, command)
    end
    ...
    def initialize(min, hour, mday, mon, wday, command)
      ...
      # wday.
      #
      @lazy_mday_wday_match = ((@mday != (1..31).to_a) && (@wday != (0..6).to_a))
    ...
    #
    def command=(c)
      raise ArgumentError, 'Commands must be a String or a Proc' unless c.is_a?(String) || c.is_a?(Proc)
      @command = c
    end
    ...
    def ready?(now = Time.now)
      ...
      if @lazy_mday_wday_match
        min.include?(now.min)  &&
          hour.include?(now.hour) &&
          mon.include?(now.mon) &&
          (mday.include?(now.mday) ||
    ...
    def next_due(now = Time.now)
      ...
      until ready?(time)
        ...
        if !mday.include?(time.mday) || @lazy_mday_wday_match
        ...
        if !wday.include?(time.wday) || @lazy_mday_wday_match
        ...
        if @lazy_mday_wday_match && time_a.is_a?(Time) && time_b.is_a?(Time)
    ...
    def run
      ...
      #
      ret = IO.popen(@command, &:readlines)
      # Check for a duff exit status.
      unless $CHILD_STATUS.success?
        ret << "Command failed with exit status #{$CHILD_STATUS.exitstatus}\n"
      end
      ret
    end
    ...
    def parse_field(str, first, last)
      str.split(',').map do |entry|
        ...
        #
        raise CrontabFormatError, "Bad entry #{entry.inspect} in field #{str.inspect}" unless entry.strip =~ /(\*|\d+)(?:\-(\d+))?(?:\/(\d+))?/

        f = Regexp.last_match(1)
        l = Regexp.last_match(2)
        every = Regexp.last_match(3)
        every = (every ? every.to_i : 1)
        raise CrontabFormatError, "Bad specifier #{every.inspect} in #{str.inspect}" if every < 1
        ...
        range = if f == '*'
                  ...
                  f, l = [f, l].collect do |n|
                    if n.is_a?(String)
                      ...
                      raise CrontabFormatError, "Bad field #{n.inspect} in #{str}" unless n =~ /^\d+$/
                      n = n.to_i
                    end

                    # Make sure we've got an integer now
                    raise CrontabFormatError, "Bad field #{n.inspect} in #{str}" unless n.is_a?(Integer)

                    # make sure everything is within ranges
                    raise CrontabFormatError, "out of range (#{n} for #{first}..#{last})" unless (first..last).cover?(n)
        ...
        range.to_a.find_all { |i| ((i - first) % every).zero? }
